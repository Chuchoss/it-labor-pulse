...
IT Labor Market Analytics (LMA) — пет/учебный проект аналитики рынка труда: сбор вакансий (HH и далее другие источники), нормализация, анализ зарплат и спроса по ролям/регионам, тренды; позже — AI-анализ; **Phase 5 Target** — секция «Тенденции» (Perspectives) на multi-source сигналах (jobs + edu + media).
