...
Проект учебный, но архитектура проектируется как production-like: чёткое разделение OLTP/OLAP, event-driven ingest, внутренний gRPC, деплой в Kubernetes.
...
Не enterprise-SLA — цели для пет-проекта, чтобы было что алертить.
