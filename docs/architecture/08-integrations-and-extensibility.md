...
- Явный `INGEST_SCOPE=it` получает официальный `/professional_roles`, выбирает
  категорию `11` («Информационные технологии») и ищет каждую её актуальную роль
  отдельно по России (`area=113`).
