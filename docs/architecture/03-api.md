...
- `is_fresh=true` только для вакансий HH, у которых `published_at` попадает в
  rolling UTC-окно `[server_now-5h, server_now]`. Обе границы включаются:
  ровно пять часов считается свежей, а пять часов и одна секунда — нет.
  `NULL` и будущие значения считаются не свежими; `first_observed_at` для
  этого флага не используется.
...
терминально переводит его в `superseded`, только если текущая immutable-версия
preferences новее версии снимка. Операция снимает lease, не удаляет решения и
счётчики и не создаёт новый run. Если снимок завершился до обработки операции,
`superseded_from_state=succeeded` сохраняет этот факт. `/status` возвращает
версии preferences, чтобы UI предлагал остановку только действительно
устаревшего снимка.
