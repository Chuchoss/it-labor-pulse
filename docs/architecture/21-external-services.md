...
| Redis (cache / locks) | [Yandex Managed Service for Valkey™](https://yandex.cloud/ru/docs/managed-valkey/) (бывш. Managed Redis) | **кандидат** (preferred для РФ) | Cache-aside, ingest locks; Phase 0 — optional BFF health ping | `REDIS_URL` (+ опц. `REDIS_TLS_CA_FILE`, `REDIS_*`) | [Yandex Valkey docs](https://yandex.cloud/ru/docs/managed-valkey/), [06-caching.md](./06-caching.md), [12 § Облачный Redis](./12-local-dev.md#облачный-redis) | Основная рекомендация при картах/доступе из РФ. Redis-compatible; TLS часто порт `6380` → `rediss://…`; для TLS нужен CA Yandex (`REDIS_TLS_CA_FILE`). Публичный доступ к хостам — иначе с ноутбука не достучаться. Тарифы меняются — смотри кабинет, **не** утверждаем free tier. BFF: `checks.redis` up/down (degraded ok). Cache path — Phase 1. Не коммитить URL |
| Redis (альтернатива, РФ) | [Selectel Managed Redis](https://docs.selectel.ru/managed-databases/redis/) | **кандидат** | Тот же `REDIS_URL`, если удобнее Selectel | `REDIS_URL` | [Selectel Redis](https://docs.selectel.ru/managed-databases/redis/) | Managed Redis; для laptop — публичный IP / публичная подсеть + ACL. Тарифы — в кабинете |
| Redis (альтернатива, РФ) | [Timeweb Cloud Redis](https://timeweb.cloud/) | **кандидат** | Тот же `REDIS_URL` | `REDIS_URL` | Кабинет Timeweb Cloud | Managed Redis у Timeweb Cloud; уточняй connection string и TLS в панели. Тарифы — в кабинете |
| Redis (альтернатива, РФ) | [VK Cloud Redis](https://cloud.vk.com/) | **кандидат** | Тот же `REDIS_URL` | `REDIS_URL` | Документация VK Cloud Databases | Managed Redis в VK Cloud; сеть/SG как у прочих MDB. Тарифы — в кабинете |
| Redis (альтернатива) | [Redis Cloud](https://redis.io/cloud/) | **кандидат** | Тот же `REDIS_URL`, если сервис доступен из твоей сети | `REDIS_URL` | [Redis Cloud](https://redis.io/cloud/) | Managed от Redis Inc.; опционально, если кабинет/endpoint открываются. Часто `rediss://` |
| Redis (не primary для РФ) | [Upstash](https://upstash.com) | **кандидат** | Serverless Redis, тот же `REDIS_URL` | `REDIS_URL` | [Upstash Docs](https://upstash.com/docs/redis) | **Может быть недоступен из РФ** (сайт/API/регистрация). Не рекомендовать как primary при работе из России. Если доступен — TLS → `rediss://…` |
| Redis (local) | Docker Compose profile `local-redis` | local fallback | Dev без cloud Redis (не основной путь) | `REDIS_URL=redis://localhost:6379/0` | [12-local-dev.md](./12-local-dev.md) | Опционально: `make up-local-redis`. Предпочтительно cloud (Yandex Valkey / др.) |
...
5. Не помечай сервис как **выбрано**, пока провайдер реально не зафиксирован командой/владельцем проекта (сейчас: Postgres → Supabase; Redis → **кандидат**, preferred для РФ — Yandex Managed Valkey/Redis).
