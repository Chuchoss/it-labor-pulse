...
вернул `match`. Доказанный `match` без неизвестных hard-фактов AI не понижает.
Неизвестный hard-критерий остаётся `review`, пока AI не вернёт валидированное
явное доказательство по каждому неизвестному критерию. `approved_roles` с
ролью `96` и `specialization=frontend` означает frontend-разработчика: official
HH catalog ID не hard-reject, если title/skills доказывают Frontend. Role gate
жёстко отсекает catalog mismatch только если специализация не задана и нет
доказательств frontend-разработчика, либо роль официально несовместима
(people-manager `104` при `include_leadership=false`). Backend/Fullstack при
запрошенном Frontend остаются `reject`. В «Подходящие» попадают `match`-
результаты текущей preference, текущего несуперседированного run или
prospective outbox и текущего ruleset: и подтверждённый AI, и deterministic
match без неизвестных hard-фактов. Повторный снимок той же preference не
скрывает ранее сохранённые match/review, пока run не суперседирован. В «Нужно
проверить» попадают оставшиеся `review` после исключения любого `reject`.
Старые результаты сохраняются для аудита, но из текущей выдачи исключаются.
