...
- Экран `/assistant` (Phase 4) задаёт роли явным multi-select по утверждённым
  ...
  provider payload.
  Результаты разделены на «Предварительно подходят по фильтрам» и
  «Подтверждено AI». AI reject/review имеет приоритет над deterministic match;
  ожидающая AI-проверки вакансия не называется финальным совпадением.
