...
- Экран `/assistant` (Phase 4) задаёт роли явным multi-select по утверждённым
  ...
  переключатель «Включать руководящие вакансии». Свободная заметка доступна в свёрнутом
  ...
  `review` после hard reject). Выдача ограничена текущими preference, run и
  ruleset. Переключатель «Включать руководящие вакансии» относится к
  management/people-lead, а не к senior/ведущий IC.
