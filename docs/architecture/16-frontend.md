...
- Экран `/assistant` (Phase 4) задаёт роли явным multi-select по утверждённым
  ...
  Для активного запуска UI отдельно показывает недоступный worker, задержку
  пакетного прогресса и безопасный статус provider backoff. Heartbeat не
  подменяет счётчик обработанных вакансий и не раскрывает provider payload.
