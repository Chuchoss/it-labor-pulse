...
- Экран `/assistant` (Phase 4) задаёт роли явным multi-select по утверждённым
  HH professional role IDs, а регионы, формат работы, минимальную зарплату и
  навыки — обычными полями формы. Для широкой роли «Разработчик» (`96`)
  появляется обязательный выбор специализации и выключенный по умолчанию
  переключатель «Включать руководящие вакансии». Подсказка у ролей: официальная
  роль HH — ориентир; при выбранной специализации решают название и навыки. Свободная заметка доступна в свёрнутом
  дополнительном блоке и не меняет deterministic matcher. Известная legacy-роль
  сопоставляется без отдельного уведомления; неизвестная требует ручного выбора.
  Запрос сохранения не содержит legacy `role`, после чего refetch-ится новая
  immutable-версия. История и архивирование доступны в свёрнутом блоке. Кнопка
  «Проверить текущие вакансии» явно сообщает об отличии ручного снимка от
  prospective-автоматизации. Рядом с AI toggle указано, что новые вакансии
  анализируются по полному описанию и возможны расходы. В очереди
  UI показывает подготовку, при обработке — `processed/total`; технические
  термины worker/outbox пользователю не показываются. Статус раздельно выводит
  deterministic reviewed, вакансии в AI, HTTP-запросы, средний пакет, AI matches,
  failures и skipped.
  При нулевых provider calls AI-совпадения отображаются как «—», отдельный
  `ai_skip_reason` объясняет причину, а завершение формулируется как «Проверка
  по критериям завершена». Если `ruleset_version` запуска отличается от
  текущего `method_version`, UI пишет «пересчёт по новым правилам» вместо
  «уже обработаны AI». Текст `already_analyzed` показывается только при том же
  ruleset и той же preference. Полным AI-анализом UI называет только run с
  фактически завершёнными provider calls.
  UI опрашивает status каждые 10 секунд даже без активного запуска и различает
  ожидание новых вакансий, обработку, retry/backoff и недоступность. Offline
  приходит готовым флагом BFF после server-side TTL, поэтому часы браузера на
  него не влияют. Для активного запуска отдельно показывается задержка пакетного
  прогресса. Heartbeat не подменяет счётчик обработанных вакансий и не раскрывает
  provider payload.
  Результаты разделены на «Подходящие» (доказанные hard-критерии: AI confirmed
  или deterministic match без неизвестных) и «Нужно проверить» (оставшийся
  `review` после hard reject). Выдача — текущие preference и ruleset, без
  суперседированных запусков. У полей фильтров короткая подсказка, с чем
  сравнивается значение (роль HH — подсказка, при специализации решают название
  и навыки; флаг remote, mid зарплаты, регион справочника, явный React).
  Переключатель «Включать руководящие вакансии» относится к management/people-lead,
  а не к senior/ведущий IC.
