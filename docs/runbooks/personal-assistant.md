...
Проверено по официальной документации 2026-08-27: модели V4 имеют context
window 1M и max output 384K, JSON Output поддерживается через
`response_format=json_object`, а disk context cache включён автоматически.
Usage содержит `prompt_tokens`, `completion_tokens`,
...
идемпотентность `(user, preference, vacancy, revision, ruleset)` действует
внутри текущего ruleset: complete-job старого ruleset не пропускает повторный
вызов. После bump ruleset снимок всегда перезаписывает deterministic решения;
в модель уходят только оставшиеся match/review. UI сравнивает
`ruleset_version` запуска с текущим `method_version` и при расхождении пишет
«пересчёт по новым правилам», а не «уже обработаны AI».
...
Если после создания ручного снимка сохранена новая версия критериев, старый
снимок не следует возобновлять. Вызовите публичную операцию
`POST /api/v1/assistant/analyze/supersede` с его `run_id`: она допустима только
при более новой preference, переводит run в терминальный `superseded`, снимает
lease и сохраняет исторические результаты и агрегаты. Новый ручной run операция
не создаёт; UI отдельно предлагает его запустить.
